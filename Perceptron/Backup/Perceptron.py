...
"""
Rozbudować klasę simple_perceptron z pliku perceptron.py (L06-L07):

a) zaimplementować bias, włączany parametrem bias=True w konstruktorze (3p)
   Podpowiedź: w perceptronie należy uwzględnić dodatkowe wejście numer z wartością 1 i zmienną wagą (podlegającą uczeniu); w03 str. 21

b) zaimplementować technikę momentum, włączaną parametrem momentum=True w konstruktorze (3p)
   Podpowiedź: w bieżącej zmianie wagi należy uwzględnić wartości zmiany wagi z poprzedniej iteracji z pewnym współczynnikiem alpha (wartość należy dobrać doświadczalnie); w05 str. 13

c) zmienić działanie opcji losowania kolejności podawania próbek treningowych (2p.)
   Podpowiedź: obecnie losowanie kolejności próbek odbywa się jednorazowo, w funkcji train_validation_split (o ile parametr shuffle=True). Zamiast tego zachowania, wprowadzić losowanie kolejności próbek przed każdą epoką treningu. Parametr shuffle powinien być argumentem konstruktora.
"""
